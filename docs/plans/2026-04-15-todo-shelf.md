...
- [x] タスクのドラッグ&ドロップ並べ替え（セクション内）
- [x] セクション CRUD UI（追加・リネーム・削除）
- [x] セクションのドラッグ&ドロップ並べ替え
...
- [x] Todoist API からデータエクスポートスクリプト作成（プロジェクト・セクション・タスク・コメント）
- [x] Shelf API への一括インポートスクリプト作成
- [x] 移行テスト（ローカル環境で実行・確認）

### 本番データ移行 [人間👨‍💻]
- [x] Todoist API トークンを取得
- [x] 移行スクリプト実行（本番 Shelf API に対して）
- [x] データ確認後、Todoist を卒業 🎉

## ログ
### 試したこと・わかったこと
- Todoist API v2 は deprecated。v1 (`/api/v1/`) を使用。レスポンス形式は `{ results: [...] }`
- 移行結果: プロジェクト3件、セクション12件、タスク61件、コメント11件
- Todoist API v1 のコメントは `task_id` ではなく `item_id` フィールド。初回移行でコメントがスキップされ、修正スクリプトで再投入した

### 移行後の改善
- 期日表示に年を追加（`1/13` → `2026/1/13`）
- タスク一覧にコメント件数バッジを追加（SVG吹き出しアイコン + 件数）。APIでタスク取得時にコメント数をJOINで返すよう変更
- セクションヘッダーのデザイン改善: 太字+白テキスト+下線でタスクと区別しやすく
- セクションの `⋯` メニュー追加（名前変更・削除を確認ダイアログ付きで実行可能に）
- タブナビを中央寄せ・フォント大きくして操作しやすく
- プロジェクト名を「TODO / アイデア / いつか」→「Todo / Deck / Vault」にリネーム
