...
### Phase 1 動作確認 [人間👨‍💻]
- [ ] Xcode で実機ビルド、同一セクション内のタスク並び替えが新D&Dで動くことを確認
- [ ] タップで詳細画面が開くことを確認（ドラッグ誤発動なし）
- [ ] ScrollView のスクロール挙動に問題なし
...
### Phase 2 動作確認 [人間👨‍💻]
- [ ] セクション間移動が動く
- [ ] 楽観更新後、サーバー再取得でもタスク順序が保たれる
- [ ] 移動中のハイライト・インジケーターが見やすい
...
### Phase 3 動作確認 [人間👨‍💻]
- [ ] swipe で削除が動く
- [ ] ドラッグ中・ドロップ時のハプティクスが心地よい
- [ ] 新D&D以外のタッチ操作（タップ・スクロール・セクションヘッダー長押し）が壊れていない

### Phase 4: オートスクロール + 視覚洗練 [AI🤖]
- [ ] ScrollView を `UIViewRepresentable` でラップし、`UIScrollView` インスタンスに DragController からアクセスできるようにする
- [ ] 指位置が上端/下端60pt以内でタイマー駆動の自動スクロール（速度: 最大 300pt/s、端に近いほど速く）
- [ ] オートスクロール中のヒットテスト再計算（LazyVStack で新しく materialize されたセクションのframe追従）
- [ ] ゴーストに `.shadow(radius: 8)` + `.scaleEffect(1.02)` で浮遊感
- [ ] 必要に応じて挿入インジケーター・背景色の微調整

### Phase 4 動作確認 [人間👨‍💻]
- [ ] 画面外のセクションまでドラッグで移動できる
- [ ] オートスクロール速度が快適
- [ ] 急なスクロール停止等の不具合なし
