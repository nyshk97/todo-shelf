...
- ~~プロジェクト追加（ツールバーメニューの「+」）~~ — iOS版ではプロジェクト管理UIを削除（タブバーと重なるため、管理はWeb版で行う）
- ~~プロジェクト名変更・削除（ツールバーメニュー）~~ — 同上

### Phase 7: 仕上げ [AI🤖]
- [x] エラーハンドリング統一（ネットワークエラー時のアラート表示）
- [x] アプリアイコン設定（棚+カラフルアイテムのデザイン）
- [x] デフォルトタブをDeckに設定
- [x] タブごとに異なるアイコン（Todo=checklist, Deck=square.stack, Vault=lock.shield）
- [x] 実機ビルド確認

### 動作確認 [人間👨‍💻]
- [x] Xcode で実機ビルド・インストール
- [x] 全機能の動作確認（本番APIに接続）
...
- `POST /tasks` や `PATCH /tasks/:id` のAPIレスポンスに `comment_count` が含まれない → Swift側で `decodeIfPresent` + デフォルト0 で対応
- Cloudflare Workers 間の fetch で `.workers.dev` ドメインに 1042 エラーが発生 → iOS側で既存todo-app APIに直接POSTする方式に変更

### 方針変更
- Phase 5 の CommentList.swift は独立ファイルではなく TaskDetailSheet 内に CommentRow として組み込んだ（画面遷移が不要なため）
- Phase 6 のプロジェクト管理は TabView の contextMenu ではなくツールバーの Menu から操作する形に変更 → 最終的にiOS版ではプロジェクト管理UI自体を削除（タブバーと重なるため、管理はWeb版で行う）
- 「今日のTODOへ移動」をサーバー間連携（Shelf API→todo-app API）からiOS直接連携に変更（Workers間fetchの制約回避）
