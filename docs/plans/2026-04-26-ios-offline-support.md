...
- 2026-06-13: 追加改善。オフライン時の `loadAll` はキャッシュ表示後にネットワーク fetch を打たず、エラー待ちを避けるよう変更
- 2026-06-13: 追加/削除/タイトル変更以外のオンライン限定操作は、API 失敗後の巻き戻しではなく事前に toast で止める方針に統一
- 2026-06-13: オンライン復帰時は `sync()` だけでなく `loadAll()` まで実行し、同期後のサーバー状態を取り直すよう変更。iOS シミュレータ Debug build pass
- 2026-06-13: レビュー指摘対応。コメント/添付削除は API 成功時だけ UI から消すよう `Bool` 返却に変更し、オフライン中はコメント行の編集/削除と添付削除 UI を出さない
- 2026-06-13: 追加レビュー指摘対応。network error 判定を retryable `URLError` に限定し、API 成功後の decode 失敗を offline create/update queue に落とさないようにした
